    ...
    stages{
        ...
        stage('Trigger playbook') {
            steps {
                 ...
                 sh "ansible-playbook -i /var/lib/jenkins/workspace/${JOB_NAME}/inventory windows.yml --extra-vars job_name=${JOB_NAME} --extra-vars build_number=${currentBuild.number}"
    }
        }    
        stage('Post action') {
            steps {
